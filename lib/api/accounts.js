...
module.exports = {
  ...
  /**
   * Updates a given payment charge settings.
   * @param {Object} params                   Object, which contains parameters of the updated account
   * @param {String} params.accountId         Account Id or house account available in current visibility scope, required
   * @param {String} params.chargeSettingsId  Account Payment Charges Setting Id, required
   * @return {Promise}                        Promise; if fulfilled returns object, which contains updated payment charge settings; if rejected returns APIerror.
   */
   ...
   * @param {String} params.accountId   Account Id or house account available in current visibility scope, required
   * @return {Promise}                  Promise; if fulfilled returns object contains compliance settings; if rejected returns APIerror.
   */
  getComplianceSettings: function (params) {
    params = params || {};
    if (!params.hasOwnProperty('accountId')) {
      throw new Error('accountId is required');
    }
    var url = '/v2/accounts/' + params.accountId + '/compliance_settings';
  ...
  /**
   * Updates compliance settings for a given account.
   * @param {Object} params                   Object, which contains parameters of the updated compliance settings
   * @param {String} params.accountId         Account Id or house account available in current visibility scope, required
   * @return {Promise}                        Promise; if fulfilled returns object, which contains updated compliance settings; if rejected returns APIerror.
   */
  updateComplianceSettings: function (params) {
    params = params || {};
    if (!params.hasOwnProperty('accountId')) {
      throw new Error('accountId is required');
    }
