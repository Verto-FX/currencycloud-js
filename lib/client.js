...
module.exports = {
  ...
  request: function (params) {
    ...
    var request = function (params) {
      ...
      // Combine request specific headers
      ...
      var promise = axios({
        ...
        paramsSerializer: function (params) {
          ...
          .then(function (response) {
            var data = utils.camelize(response.data);
            // Include headers in response when requested
            if(params.returnResponseHeaders && data && typeof data === 'object') {
    ...
    var promise = request(params)
        .catch(function (res) {
          if (res.response.status && res.response.status === 401 && token) {
